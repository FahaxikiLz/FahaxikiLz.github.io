...
### 为什么使用了promise异步，还要使用awit和async？

> 使用Promise可以解决回调地狱的问题，但是在某些情况下，Promise的使用方式仍然会很麻烦，比如需要等待多个Promise完成后再执行一些操作，类似于并发请求。此时，可以使用async/await，它提供了一种更加直观、简洁的异步操作方式，能更好地处理Promise的异步操作，提高代码可读性和可维护性。同时，使用await可以等待Promise的结果返回，优雅地解决回调地狱问题。所以，在某些情况下，使用Promise和async/await会更加便捷。
