...
`@Import` 注解在 Spring Boot 和 Spring 框架中用于导入一个或多个 `@Configuration` 类、`ImportSelector`、`ImportBeanDefinitionRegistrar`，或者普通的组件类。通过 `@Import` 注解，您可以将这些组件的定义加载到 Spring 应用上下文中。以下是一些具体的使用示例和解释：

###### 1. 导入一个或多个 `@Configuration` 类

`@Configuration` 类用于定义 Spring 应用上下文中的 Bean。通过 `@Import` 注解，您可以将其他 `@Configuration` 类导入当前配置类中。
...
public class MainConfig {
    // MainConfig 现在也会加载 AppConfig 中定义的 Bean
}
```

###### 2. 导入 `ImportSelector`

`ImportSelector` 是一个接口，允许您以编程方式选择和导入多个配置类。
...
public class MainConfig {
    // MainConfig 通过 MyImportSelector 导入了 AppConfig
}
```

###### 3. 导入 `ImportBeanDefinitionRegistrar`

`ImportBeanDefinitionRegistrar` 允许您以编程方式注册更多的 Bean 定义。
...
public class MainConfig {
    // MainConfig 通过 MyImportBeanDefinitionRegistrar 注册了 MyService Bean
}
```

###### 4. 导入普通组件类

除了导入 `@Configuration` 类，`@Import` 注解还可以直接导入普通的组件类。
...
public class MainConfig {
    // MainConfig 现在也会加载 MyComponent 作为一个 Bean
}
```

通过 `@Import` 注解，可以灵活地组合和管理应用程序中的配置和组件，提高代码的模块化和可维护性。

##### @ImportResource

> 解决问题：一些公司还是使用xml文件配置方式，所以需要将这些文件引进来，进行解析配置

![](SpringBoot/20200703150815127.png)

> 不使用`@ImportResource()`注解，程序根本不能对我们spring的配置文件进行加载，所以我们需要将spring配置文件加载到容器里。

![](SpringBoot/20200703151227559.png)
...
`@ConditionalOnProperty` 注解是 Spring Boot 提供的一个条件注解，用于基于配置文件中的属性值来控制 Bean 的注册。这使得在不同的环境或配置条件下，有选择地加载或不加载特定的 Bean。

**主要用途**

1. **根据配置条件创建 Bean**：只在特定的属性值满足时，才创建 Bean。
2. **简化配置管理**：允许在配置文件中控制 Bean 的创建，从而避免在代码中硬编码条件。

**用法**

`@ConditionalOnProperty` 可以用于 `@Configuration` 类、`@Bean` 方法或其他 `@Component` 类。

###### @ConditionalOnProperty简介
