...
* SocketChannel（用于客户端/服务端TCP）
* ServerSocketChannel（用于服务端TCP）
...
public class ChannelDemo1 {
    public static void main(String[] args) {
        try (FileChannel channel = new FileInputStream("data.txt").getChannel()) {
            /*准备byte buffer*/
            ByteBuffer byteBuffer = ByteBuffer.allocate(10);// 设置容量为10
            int read;
            while ((read = channel.read(byteBuffer)) != -1) {
                log.debug("读到字节数：{}",read);
...
![](Netty/0021-1701068782748.png)

写模式下，position 是写入位置，limit 等于容量，下图表示写入了 4 个字节后的状态

![](Netty/0018-1701068782749.png)

flip 动作发生后，position 切换为读取位置，limit 切换为读取限制

![](Netty/0019-1701068782749.png)

读取 4 个字节后，状态

![](Netty/0020-1701068782749.png)
...
可以**使用 allocate 方法为 ByteBuffer 分配空间**，其它 buffer 类也有该方法

```java
Bytebuffer buf = ByteBuffer.allocate(16);// java堆内存，读写效率较低，受GC影响
ByteBuffer byteBuffer = ByteBuffer.allocateDirect(10);// 直接内存，读写效率较高（少一次拷贝），不受GC影响，分配的效率低
```
...
public static void main(String[] args) {
        ...
        debugAll(buffer1);// 会一直保持写模式

        /*Charset*/
        ByteBuffer buffer2 = StandardCharsets.UTF_8.encode("hello");
        debugAll(buffer2);// 写入之后会转成读取模式

        ByteBuffer buffer3 = Charset.forName("UTF-8").encode("hello");
        debugAll(buffer3);// 写入之后会转成读取模式

        /*wrap*/
        ByteBuffer buffer4 = ByteBuffer.wrap("hello".getBytes());
        debugAll(buffer4);// 写入之后会转成读取模式


        /*转成字符串*/
        String str1 = StandardCharsets.UTF_8.decode(buffer1).toString();
        /*buffer1写入之后还保持的写入模式，当转成字符串的时候position位于写入的位置，读取会读不到数据*/
        System.out.println("str1 = " + str1);//str1 =
        String str2 = StandardCharsets.UTF_8.decode(buffer2).toString();
        /*buffer2写入之后就会转成读取模式，当转成字符串的时候position位于0的位置就可以读取到数据*/
        System.out.println("str2 = " + str2);//str2 = hello
    
        String str3 = Charset.forName("UTF-8").decode(buffer3).toString();
...
public static void main(String[] args) {
        ...
        try (FileChannel channel = new RandomAccessFile("src/main/resources/3parts.txt", "rw").getChannel()) {
...
但由于某种原因这些数据在接收时，被进行了重新组合，例如原始数据有4条为
...
```java
	/*遍历文件和文件夹*/
    private static void goThroughFilesAndFolders() {
        AtomicInteger dirCount = new AtomicInteger();
        AtomicInteger fileCount = new AtomicInteger();

        try {
            Files.walkFileTree(Paths.get("F:\\尚硅谷文件\\JDBC及DAO设计"), new SimpleFileVisitor<Path>() {
...
```java
	/*遍历查看mp4文件*/
    private static void walkThroughTheMp4File() {
        AtomicInteger mp4Count = new AtomicInteger();
            ...
            Files.walkFileTree(Paths.get("F:\\尚硅谷文件\\JDBC及DAO设计"), new SimpleFileVisitor<Path>() {
                ...
                public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
                    if (file.toString().endsWith(".mp4")) {
                        mp4Count.incrementAndGet();
                        System.out.println("mp4 = " + file.toString());
                    }
                    return super.visitFile(file, attrs);
                }
            });

            System.out.println("mp4Count = " + mp4Count);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
...
```java
	/*删除多级目录*/
    private static void deleteTheMultiLevelDirectory() throws IOException {
        Files.walkFileTree(Paths.get("F:\\尚硅谷文件\\JDBC及DAO设计"),new SimpleFileVisitor<Path>(){
...
删除是危险操作，确保要递归删除的文件夹没有重要内容，删除的文件不会放在回收站
...
```java
	/*拷贝多级目录*/
    private static void copyAMultiLevelDirectory() throws IOException {
        String source = "F:\\尚硅谷文件\\JDBC及DAO设计";
        String target = "F:\\target\\尚硅谷文件\\JDBC及DAO设计";
...
public class ChannelDemo6 {
    public static void main(String[] args) {
        try (ServerSocketChannel channel = ServerSocketChannel.open()) {
            ...
            while (true) {
                int count = selector.select(); // 返回selector中的channel数量
//                int count = selector.selectNow();
...
byte的最大容量为4个字节，一个英文字母占1个字节，在UTF-8编码下一个汉字占3个字节，一个单词就有可能一次输出一部分的情况，一个汉字可能被截取几个字节的情况。



#### 处理消息的边界

![](Netty/0023-1701068782749.png)
...
![](Netty/0033-1701068782749.png)

* 阻塞 IO

  ![](Netty/0039-1701068782749.png)

* 非阻塞  IO

  ![](Netty/0035-1701068782749.png)

* 多路复用

  ![](Netty/0038-1701068782749.png)

* 信号驱动

* 异步 IO

  ![](Netty/0037-1701068782749.png)

* 阻塞 IO vs 多路复用

  ![](Netty/0034-1701068782749.png)

  ![](Netty/0036-1701068782749.png)
...
>
>   ![](Netty/0049-1701068856278.png)
>
> 
>
> * 为了解决此问题，引入了窗口概念，窗口大小即决定了无需等待应答而可以继续发送的数据最大值
>
>   ![](Netty/0051-1701068856279.png)
>
...
>
>   <img src="Netty/0031.jpg" style="zoom:50%;" />
