...
代码片段中`getName()`方法中的`name参数`用于接收子组件传递过来的参数
...
![2.png](vue/5b2adcc1a6e34a8884780212281bbc67_tplv-k3u1fbpfcp-zoom-in-crop-mark_1512_0_0_0.webp)

点击按钮后：

![3.png](vue/9ad0a8cdb57841b98f5c68ca7123192b_tplv-k3u1fbpfcp-zoom-in-crop-mark_1512_0_0_0.webp)

通过图上可以看出，当点击按钮之后，控制台输出了父组件收到的数据，子组件通过`props`的方式向父组件传递了数据
...
![4.png](vue/50840c8ec207417b9036144d33c23562_tplv-k3u1fbpfcp-zoom-in-crop-mark_1512_0_0_0.webp)

其次，在子组件中，需要对自定义事件进行触发，完成组件自定义事件通信:

![5.png](vue/fbec69676fd64707869fb2a139aae4a7_tplv-k3u1fbpfcp-zoom-in-crop-mark_1512_0_0_0.webp)

**效果图如下：**

页面初始化效果：

![6.png](vue/d3f99c5ae4634ffb8baebdd318de3bf4_tplv-k3u1fbpfcp-zoom-in-crop-mark_1512_0_0_0.webp)

点击按钮后：

![7.png](vue/3a69fbdf4756465c91b63b9581f7b6aa_tplv-k3u1fbpfcp-zoom-in-crop-mark_1512_0_0_0.webp)

通过图上可以看出，当点击按之后，控制台输出了父组件收到的数据。

通过以上两种通信方式，我们可以发现，子组件通过`props`方式向父组件传递数据，**前提是父组件要给子组件传递一个回调函数**，子组件接收之后，才能向父组件传递数据，而组件的自定义事件只需调用 `$emit` 方法对指定自定义事件**进行触发**，即可向父组件传递数据。
...
通过`ref`属性拿到`TestB组件组件的实例对象（vc）`，在组件挂载完成之后（mounted）使用`this.$refs.组件名.$on('自定义事件名', 回调函数)`完成对子组件自定义事件的绑定，同样也能实现效果。
...
直接不用传递任何参数，这样写的话，只要是给此组件绑定的**任何自定义事件都会解绑**。



作者：Over
链接：https://juejin.cn/post/7083368269201031198
来源：稀土掘金
