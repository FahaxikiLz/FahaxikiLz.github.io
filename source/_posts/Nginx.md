...
> - **`proxy_pass` 的行为**
>
>   - **`proxy_pass` 以斜杠 (`/`) 结尾**：如果 `proxy_pass` 后面的 URL 以 `/` 结尾，那么 Nginx 会剥离 `location` 中匹配的部分，并仅将剩余的 URI 部分附加到 `proxy_pass` 后面的 URL。
>   - **`proxy_pass` 不以斜杠 (`/`) 结尾**：如果 `proxy_pass` 后面的 URL 不以 `/` 结尾，Nginx 会保留 `location` 中匹配的部分，并将整个请求 URI 直接附加到 `proxy_pass` 的 URL 后面。
>
>   具体配置对比
>
>   1. **`proxy_pass http://127.0.0.1:8081/;`**：
>      - 请求 `/edu/anything` 会被转发为 `http://127.0.0.1:8081/anything`。
>      - `/edu/` 部分被剥离，不包含在最终请求路径中。
>   2. **`proxy_pass http://127.0.0.1:8081;`**：
>      - 请求 `/edu/anything` 会被转发为 `http://127.0.0.1:8081/edu/anything`。
>      - `/edu/` 部分保留，并包含在最终请求路径中。
...
>
> - root和alias的区别：
>   - `root` 指令会将 `location` 块中匹配的 URI 部分与 `root` 指定的路径拼接起来。
>   - `alias` 指令将 `location` 后面的 URI 直接替换为 `alias` 指定的路径。
