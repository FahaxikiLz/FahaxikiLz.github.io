---
title: ShardingSphere5
date: 2025-05-21 10:11:49
tags:
- ShardingSphere5
categories:
- 中间件
...
![img](ShardingSphere5/362d22168bf344687ec0c206aa115807.jpg)
...
![image-20220804223138651](ShardingSphere5/image-20220804223138651.png)
...
- 在实际设计过程中，每个系统不可能只处理一种数据，而是包含多种类型的数据，`有的数据必须选择 CP，有的数据必须选择 AP，分布式系统理论上不可能选择 CA 架构。`
  ...
  ![img](ShardingSphere5/6e7d7bd54d7a4eb67918080863d354d7.png)
...
![img](ShardingSphere5/2ccafe41de9bd7f8dec4658f004310d6.png)
...
![img](ShardingSphere5/71f41d46cc5c0405f4d4dc944b4350c9.jpg)

下图展示了根据业务需要，将用户表和订单表垂直分片到不同的数据库的方案：

![image-20220804221855449](ShardingSphere5/image-20220804221855449.png)
...
![img](ShardingSphere5/136bc2f01919edcb8271df6f7e71af40.jpg)
...
![image-20220804222212087](ShardingSphere5/image-20220804222212087.png)
...
![image-20220804223321167](ShardingSphere5/image-20220804223321167.png)
...
![img](ShardingSphere5/f8d538f9201e3ebee37dfdcd1922e9df.jpg)
...
![img](ShardingSphere5/2a2dba7f07581fd055d9cd5a3aa8388e.jpg)
...
![image-20220804195402870](ShardingSphere5/image-20220804195402870.png)
...
![image-20220804195432673](ShardingSphere5/image-20220804195432673.png)
...
![img](ShardingSphere5/image-20220714133617856.png)
...
![image-20220807183231101](ShardingSphere5/image-20220807183231101.png)
...
![img](ShardingSphere5/0.08703112216569037.png)
...
![image-20220804191852164](ShardingSphere5/image-20220804191852164.png)
...
![img](ShardingSphere5/image-20220715000533951.png)
...
![img](ShardingSphere5/image-20220714235426120.png)
...
![img](ShardingSphere5/image-20220715004850504.png)
...
![image-20220807232456342](ShardingSphere5/image-20220807232456342.png)
...
![image-20220810163534068](ShardingSphere5/image-20220810163534068.png)
...
![image-20220808033239206](ShardingSphere5/image-20220808033239206.png)
...
![image-20220819152009158](ShardingSphere5/image-20220819152009158.png)
...
![image-20220819151154763](ShardingSphere5/image-20220819151154763.png)
...
- 比如按业务垂直分库，将订单，用户信息分成两个库ds0 ds1，然后ds0 ds1分别有一个主库一个从库
- 比如将订单按id分库分表，分成两个库ds0 ds1，然后ds0 ds1分别有一个主库一个从库

在 **分库分表** 的配置里，你永远只引用 **逻辑数据源名**（如 `ds0`、`ds1`），**不** 是直接写 `ds0_master` 或 `ds0_slave`。

- 先在读写分离里把一对主-从物理源（`ds0_master` + `ds0_slave`）封装成一个逻辑源 `ds0`；同理再把 `ds1_master` + `ds1_slave` 封装成 `ds1`。也就是下面的`my_order0`/`my_order1`
- 然后在分库分表的 `actual-data-nodes`、`database-strategy`、`table-strategy` 里只写 `ds0`、`ds1`。

```properties
# 1. 先定义物理数据源（不变）
spring.shardingsphere.datasource.names=\
  order0_master,order0_slave1,order0_slave2,\
  order1_master,order1_slave1,order1_slave2

# —— 分片0 的主从 —— 
spring.shardingsphere.datasource.order0_master.jdbc-url=jdbc:mysql://192.168.100.201:3306/order_db_0
…（用户名、密码同理）…
spring.shardingsphere.datasource.order0_slave1.jdbc-url=jdbc:mysql://192.168.100.201:3307/order_db_0
spring.shardingsphere.datasource.order0_slave2.jdbc-url=jdbc:mysql://192.168.100.201:3308/order_db_0

# —— 分片1 的主从 —— 
spring.shardingsphere.datasource.order1_master.jdbc-url=jdbc:mysql://192.168.100.202:3306/order_db_1
…  
# 2. 读写分离：写分离里把一对主-从物理源（`ds0_master` + `ds0_slave`）封装成一个逻辑源 `ds0`，也就是此处的my_order0/my_order1
spring.shardingsphere.rules.readwrite-splitting.data-sources.my_order0.type=Static
spring.shardingsphere.rules.readwrite-splitting.data-sources.my_order0.props.write-data-source-name=order0_master
spring.shardingsphere.rules.readwrite-splitting.data-sources.my_order0.props.read-data-source-names=order0_slave1,order0_slave2

spring.shardingsphere.rules.readwrite-splitting.data-sources.my_order1.type=Static
spring.shardingsphere.rules.readwrite-splitting.data-sources.my_order1.props.write-data-source-name=order1_master
spring.shardingsphere.rules.readwrite-splitting.data-sources.my_order1.props.read-data-source-names=order1_slave1,order1_slave2

# 负载均衡
spring.shardingsphere.rules.readwrite-splitting.load-balancers.round.type=ROUND_ROBIN
# 3. 分库分表：引用新逻辑源 my_order0、my_order1
spring.shardingsphere.rules.sharding.tables.t_order.actual-data-nodes=\
  my_order$->{0..1}.t_order_$->{0..1}

# 分库策略：user_id % 2 → my_order0 / my_order1
spring.shardingsphere.rules.sharding.tables.t_order.database-strategy.standard.sharding-column=user_id
spring.shardingsphere.rules.sharding.tables.t_order.database-strategy.standard.sharding-algorithm-name=db_inline

# 分表策略：order_id % 2 → t_order_0 / t_order_1
spring.shardingsphere.rules.sharding.tables.t_order.table-strategy.standard.sharding-column=order_id
spring.shardingsphere.rules.sharding.tables.t_order.table-strategy.standard.sharding-algorithm-name=tb_inline

# 算法定义
spring.shardingsphere.rules.sharding.sharding-algorithms.db_inline.type=INLINE
spring.shardingsphere.rules.sharding.sharding-algorithms.db_inline.props.algorithm-expression=\
my_order$->{user_id % 2}

spring.shardingsphere.rules.sharding.sharding-algorithms.tb_inline.type=INLINE
spring.shardingsphere.rules.sharding.sharding-algorithms.tb_inline.props.algorithm-expression=\
t_order_$->{order_id % 2}
```

> **要点**：
>
> - 逻辑数据源名（原 `order_ds0`/`order_ds1`）随便起，这里改成 `my_order0`、`my_order1`。
> - 分库分表规则中的 `actual-data-nodes`、`algorithm-expression`、以及读写分离的 `data-sources.*` 全部同步改为新逻辑名。
> - 写 SQL 时先选逻辑库（`my_order0`/`my_order1`），再根据执行类型（读/写）自动路由到对应的 `*_master` 或 `*_slaveX`。

这样就完成了“把 `order_ds0` 换成 `my_order`”并继续做分库分表和读写分离。